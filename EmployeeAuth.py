...
with open('myDataFile.text') as f:      # opening myDataFile; file that contains authenticated person's ids
    myDataList = f.read().splitlines()  # it will read all the data and based on the lines it will add 1 item to the
    # list, so every line is a new item
print(myDataList)
...
while True:
    ...
    for barcode in decode(img):
        ...
        if myData in myDataList:    # checking if the data is present in myDataList
            ...
            # playsound("C:\\Users\\sachi\\PycharmProjects\\EmployeeAuthenticationSystem\\unauthenticated.mp3")    # play sound for un-authorized
